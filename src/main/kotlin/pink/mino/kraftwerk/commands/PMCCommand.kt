        ...
        if (player.scoreboard.getPlayerTeam(player) != null) {
            for (team in player.scoreboard.getPlayerTeam(player).players) {
                if (team is Player) {
                    ...
                    Chat.sendMessage(team, "§8[§4Team Chat§8] ${ChatColor.WHITE}${sender.name} ${Chat.dash} &a${player.name}'s&7 location: &c${x}&7, &c${y}&7, &c${z} &8| &7Dimension: &c${player.world.worldType.toString().uppercase()}")
                }
            }
        }
        return true
