...
class MoleLocationCommand : CommandExecutor {
    override fun onCommand(
        ...
        if (!ScenarioHandler.getActiveScenarios().contains(ScenarioHandler.getScenario("moles"))) {
            Chat.sendMessage(sender, "${Chat.prefix} &aMoles&7 isn't enabled!")
            return false
        }
        if (GameState.currentState != GameState.INGAME) {
            Chat.sendMessage(sender, "${Chat.prefix} &aMoles&7 isn't available right now!")
            return false
        }
        ...
        MolesScenario.instance.sendMoles("&8[&aMole Chat&8]&f ${PlayerUtils.getPrefix(sender)}${sender.name}&7's location: &f${x}, ${y}, ${z}&7. &8| &7Dimension: &a${sender.location.world.worldType}&7.")
        return true
    }
}